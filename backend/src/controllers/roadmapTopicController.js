...
/**
 * @desc    Lấy tất cả lộ trình của user
 * @route   GET /api/roadmap-topic/all
 * @access  Private
 */
exports.getAllRoadmaps = async (req, res) => {
  try {
    ...
    const roadmaps = await LearningRoadmap.find({ user: userId })
      .sort({ createdAt: -1 })
      .select('topic level overallProgress steps isActive createdAt');
...
/**
 * @desc    Lấy lộ trình theo ID
 * @route   GET /api/roadmap-topic/:id
 * @access  Private
 */
exports.getRoadmapById = async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      message: 'Không thể lấy lộ trình',
      error: error.message
    });
  }
};
